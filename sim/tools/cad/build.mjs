...
function material(mat, fromAero, leaf = "") {
  ...
  for (const o of OVERRIDES) {
    if ((!fromAero || o.aero) && o.test.test(seg)) {
  ...
  const c = outColour(mat, fromAero).map((v) => Math.round(v * 50) / 50);
  const metal = /steel|alumin|gold/.test((mat?.getName() ?? "").toLowerCase());
...
async function ingest(file, { fromAero, skip }) {
  ...
  src.getRoot().listScenes()[0].traverse((node) => {
    ...
    const tg = (/^Mirror/.test(leaf.split("/").pop())
      ...
      ?? (!fromAero && (lo[1] + hi[1]) / 2 < -0.02 ? leftSide : null);
    for (const pr of prims) {
...
outScene.setExtras({ suspension: { frame: "cad: x forward from the front axle, y left, z up; metres", corners: hardpoints } });
...
await out.transform(join({ keepNamed: false, filter: (n) => !/^(wheel_|rig:)/.test(n.getName()) }), prune());
...
console.log(JSON.stringify({
  ...
  splitFinish: report.splitFinish, parts: report.kept, dropped: report.dropped, dupes: report.dupes, reflected: report.reflected, rig: report.rig,
